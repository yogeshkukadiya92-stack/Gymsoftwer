...
import { starterLeads } from "@/lib/business-data";

export default function AdminCrmPage() {
  const newLeads = starterLeads.filter((lead) => lead.status === "New").length;
  const convertedLeads = starterLeads.filter((lead) => lead.status === "Converted").length;
  const trials = starterLeads.filter((lead) => lead.status === "Trial Booked").length;
  ...
  return (
    ...
    >
      <div className="grid gap-6 lg:grid-cols-4">
        <StatCard label="Total leads" value={String(starterLeads.length)} detail="All active inquiry records in your CRM." />
      ...
      <div className="mt-6">
        <SectionCard eyebrow="Pipeline" title="Lead board">
          <LeadCrmWorkspace leads={starterLeads} />
        </SectionCard>
      </div>
    </AppShell>
  );
